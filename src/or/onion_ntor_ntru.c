...
#include "crypto.h"
#define ONION_NTOR_NTRU_PRIVATE
...
void
ntor_ntru_handshake_state_free(ntor_ntru_handshake_state_t *state)
{
  if (!state)
    return;
...
const tweakset_t ntor_ntru_tweaks = {
#define PROTOID "ntor-curve25519-sha256-1-ntruees439ep1"
#define PROTOID_LEN 38
  PROTOID ":mac",
  PROTOID ":key_extract",
  PROTOID ":verify",
  PROTOID ":key_expand"
 ...
 * and whose onion key is <b>router_key</b>. Store the
 * NTOR_NTRU_ONIONSKIN_LEN-byte message in <b>onion_skin_out</b>, and store the
 * handshake state in <b>handshake_state_out</b>.
 * Return 0 on success, -1 on failure.
 */
int
onion_skin_ntor_ntru_create(const uint8_t *router_id,
                            const curve25519_public_key_t *router_key,
                            ntor_ntru_handshake_state_t **handshake_state_out,
                            uint8_t *onion_skin_out)
{
  ntor_ntru_handshake_state_t *state;
  uint8_t *op;

  state = tor_malloc_zero(sizeof(ntor_ntru_handshake_state_t));
  ...
  if (ntruees439ep1_keypair_generate(&state->pubkey_N, &state->seckey_n)) {
    tor_free(state);
    return -1;
  }
  ...
  APPEND(op, state->pubkey_N.public_key, NTRUEES439EP1_PUBKEY_LEN);
  tor_assert(op == onion_skin_out + NTOR_NTRU_ONIONSKIN_LEN);
...
#define SECRET_INPUT_LEN (CURVE25519_OUTPUT_LEN * 2 +      \
                          ...
                          NTOR_NTRU_ENCAPSULATED_KEY_LEN + \
                          NTRUEES439EP1_PUBKEY_LEN +       \
                          PROTOID_LEN)

#define AUTH_INPUT_LEN (DIGEST256_LEN + DIGEST_LEN +    \
                        CURVE25519_PUBKEY_LEN*3 +       \
                        NTRUEES439EP1_CIPHERTEXT_LEN +  \
                        NTRUEES439EP1_PUBKEY_LEN +      \
                        PROTOID_LEN + SERVER_STR_LEN)

/**
 * Perform the server side of an ntor handshake. Given an
 * NTOR_NTRU_ONIONSKIN_LEN-byte message in <b>onion_skin</b>, our own identity
 ...
 * indicates an unrecognized public key.  Write an NTOR_NTRU_REPLY_LEN-byte
 ...
 */
int
onion_skin_ntor_ntru_server_handshake(const uint8_t *onion_skin,
                                      ...
                                      size_t key_out_len)
{
  const tweakset_t *T = &ntor_ntru_tweaks;
  ...
  struct {
    ...
    uint8_t parallel_secret[NTOR_NTRU_ENCAPSULATED_KEY_LEN];
    ntruees439ep1_ciphertext_t parallel_ciphertext;
  } s;
  ...
  memcpy(s.pubkey_N.public_key,
         onion_skin+DIGEST_LEN+DIGEST256_LEN+CURVE25519_PUBKEY_LEN,
         NTRUEES439EP1_PUBKEY_LEN);
  ...
  /* Make s, c */
  if (crypto_rand((char*)s.parallel_secret, sizeof(s.parallel_secret)) < 0)
    bad |= 1;

  bad |= ntruees439ep1_encrypt(&s.pubkey_N, NTOR_NTRU_ENCAPSULATED_KEY_LEN,
                               (const uint8_t*)&s.parallel_secret,
                               &s.parallel_ciphertext);
  ...
  APPEND(si, s.parallel_secret, NTOR_NTRU_ENCAPSULATED_KEY_LEN);
  APPEND(si, s.pubkey_N.public_key, NTRUEES439EP1_PUBKEY_LEN);
  ...
  APPEND(ai, s.parallel_ciphertext.ciphertext, NTRUEES439EP1_CIPHERTEXT_LEN);
  APPEND(ai, s.pubkey_N.public_key, NTRUEES439EP1_PUBKEY_LEN);
  ...
  memcpy(handshake_reply_out+CURVE25519_PUBKEY_LEN,
         s.parallel_ciphertext.ciphertext, NTRUEES439EP1_CIPHERTEXT_LEN);
  h_tweak(handshake_reply_out+CURVE25519_PUBKEY_LEN+NTRUEES439EP1_CIPHERTEXT_LEN,
                           ...
                           size_t key_out_len)
{
  ...
  memcpy(s.parallel_ciphertext.ciphertext,
         handshake_reply+CURVE25519_PUBKEY_LEN, NTRUEES439EP1_CIPHERTEXT_LEN);
  auth_candidate =
    handshake_reply+CURVE25519_PUBKEY_LEN+NTRUEES439EP1_CIPHERTEXT_LEN;

  /* Decrypt parallel ciphertext */
  bad |= ntruees439ep1_decrypt(&handshake_state->seckey_n,
                               &s.parallel_ciphertext,
                               NTOR_NTRU_ENCAPSULATED_KEY_LEN,
                               s.parallel_secret);
  ...
  APPEND(si, s.pubkey_Y.public_key, CURVE25519_PUBKEY_LEN);
  APPEND(si, s.parallel_secret, NTOR_NTRU_ENCAPSULATED_KEY_LEN);
  APPEND(si, handshake_state->pubkey_N.public_key, NTRUEES439EP1_PUBKEY_LEN);
  APPEND(si, PROTOID, PROTOID_LEN);
  tor_assert(si == s.secret_input + sizeof(s.secret_input));
  ...
  APPEND(ai, s.parallel_ciphertext.ciphertext, NTRUEES439EP1_CIPHERTEXT_LEN);
  APPEND(ai, handshake_state->pubkey_N.public_key, NTRUEES439EP1_PUBKEY_LEN);
  APPEND(ai, PROTOID, PROTOID_LEN);
  APPEND(ai, SERVER_STR, SERVER_STR_LEN);
  tor_assert(ai == s.auth_input + sizeof(s.auth_input));
  ...
  if (bad) {
    log_warn(LD_PROTOCOL,
             "Invalid result from curve25519 or NTRU handshake: %d", bad);
  }
