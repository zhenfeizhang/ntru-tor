...
#ifndef NTOR_NTRU_ENABLED
#error "ntor+ntru handshake is not enabled."
#endif
...
{
  uint8_t msg_in[NTOR_NTRU_ONIONSKIN_LEN];
  curve25519_keypair_t kp;
  di_digest256_map_t *keymap=NULL;
  uint8_t node_id[DIGEST_LEN];
  int keybytes;

  uint8_t msg_out[NTOR_NTRU_REPLY_LEN];
  uint8_t *keys = NULL;
  char *hexkeys = NULL;
  int result = 0;
  ...
  BASE16(4, msg_in, NTOR_NTRU_ONIONSKIN_LEN);
  INT(5, keybytes);
  ...
  if (onion_skin_ntor_ntru_server_handshake(
